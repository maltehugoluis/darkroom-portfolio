...
function DarkroomContent() {
  ...
  // Speichert den Canvas-Stand und lädt die Kategorie
  const selectCategory = async (label: string, showLoading: boolean = true) => {
    playAutofocusSound();
      ...
      {!currentCategory ? (
        ...
        <div className="p-4 md:p-16 h-full flex flex-col justify-center items-center relative bg-black text-center">
          ...
          <div className="flex flex-col items-center gap-6 md:gap-8 w-full max-w-xs md:max-w-none mb-24 font-mono">
            ...
            <button
              ...
              className="text-xs md:text-xl font-mono text-zinc-500 tracking-[0.2em] uppercase transition-all duration-300 hover:text-red-600 outline-none cursor-pointer border-none bg-transparent disabled:opacity-50"
            >
              {isGeneratingPDF ? (pdfProgressText || "PDF WIRD BELICHTET...") : "PORTFOLIO PDF (DOWNLOAD)"}
            </button>
          </div>
        ...
        <>
          ...
          {!loading && currentCategory && currentCategory !== "KONTAKT" && (
            <div className="hidden md:flex fixed bottom-6 left-1/2 -translate-x-1/2 z-[150] w-[94%] max-w-xl bg-black/85 backdrop-blur-md border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.8)] rounded-full px-3 py-2 items-center justify-between gap-2 font-mono overflow-hidden">
              ...
              {/* Frame Counter (Desktop & Tablet) */}
              {images.length > 0 && (
                <div className="hidden sm:flex items-center gap-2 shrink-0 text-[10px] md:text-[11px] text-zinc-400 tracking-widest uppercase pl-1">
